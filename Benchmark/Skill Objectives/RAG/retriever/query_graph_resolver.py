...
class QueryGraphResolver:
    ...
    async def entity_linker_by_id(self, entity_id: str) -> str:
        ...
        return None
    async def entity_description_by_id(self, entity_id: str) -> str:
        """
        根据实体 ID 返回该实体的属性描述文本
        """
        cypher = "MATCH (a) WHERE a.名称 = $entity_id RETURN a"
        result = self.client.run(cypher, entity_id=entity_id)

        if result and isinstance(result, list):
            node = result[0].get("a")  # 取出节点本体（dict）
            if isinstance(node, dict):
                description = f"实体『{entity_id}』的属性如下：\n"
                for key, value in node.items():
                    description += f"- {key}：{value}\n"
                return description.strip()
        return f"未找到实体 ID 为『{entity_id}』的节点。"
    ...
    async def resolve(self, query_graph: Dict) -> Dict:
        ...
        for entity in entities:
            entity['description']=await self.entity_description_by_id(entity['name'])
